...
- 새로 작성하거나 수정한 Java/Kotlin 메서드는 들여쓰기 깊이를 최대 2단계로 제한한다.
  `if/for/try`가 3단계 이상 중첩되기 시작하면 early return, guard clause, private helper,
  SQL/MyBatis 위임, 작은 객체 책임 분리 중 하나로 흐름을 평탄화한다.
